<script lang="ts">
  ...
  let editingBoardKey = $state<string | null>(null);
  ...
  function applyBoardEdit() {
    ...
    boardLog = boardLog.map((e) =>
      `${e.set}-${e.board}-${e.endedAt}` === key
        ? { ...e, pointsA: pA, pointsB: pB, queen: editDraft!.queen }
        : e,
    );
...
<section class="wrap">
  ...
  {#if showRecapPopup}
    <!--
      Edit scores popup (v3.5). Uses LiveScoreboardView with optional edit
      props so board rows are tappable when the match is still in progress.
      Tapping a row expands it inline with score inputs + queen toggle.
    -->
    {@const recapRecord = {
      matchId: '',
      updatedAt: Date.now(),
    ...
    >
      <div class="dialog-card scorecard-card">
        ...
        <LiveScoreboardView
          record={recapRecord}
          editingBoardKey={editingBoardKey}
          editDraft={editDraft}
          onEditStart={!isMatchDecided() && !isPractice
            ? (key, draft) => { editingBoardKey = key; editDraft = draft; }
            : undefined}
...
<style>
  ...
  .coin-callout::after {
    ...
    border-bottom-color: rgba(255, 213, 74, 0.7);
  }
  @keyframes coinPulse {
  ...
  .coin-btn {
    ...
    filter: drop-shadow(0 0 6px rgba(220, 30, 30, 1)) drop-shadow(0 0 12px rgba(200, 20, 20, 0.8));
  }
